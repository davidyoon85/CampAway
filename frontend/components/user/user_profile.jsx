...
function UserProfile(props) {
  ...
  useEffect(() => {
    ...
    props.fetchAllSpots();
    getAddressInfoByZip(props.currentUser.zip_code.toString());
  }, []);

  function tripsButton(e) {
    e.preventDefault();
    if (trips === false) {
      setTrips(true);
      setReviews(false);
  ...
  function getAddressInfoByZip(zip) {
    geocodeByAddress(zip)
      .then(results => {
        ...
        setState(results[0].address_components[2].short_name);
        return;
      })
      .catch(err => {
  ...
  function getUserReviews() {
    ...
    const userReviews = reviews.filter(
      spot => spot.author == props.match.params.userId
    );

    return userReviews;
  }

  const { bookings, deleteReview, deleteBooking } = props;
  let userReviews = getUserReviews();
  let numTrips = Object.keys(bookings).length;
  let tripName = "";
  let numReviews = userReviews.length;
  let reviewName = "";
  let startDate = format(props.currentUser.created_at, "MMMM YYYY");
        ...
        <div className="user_booking_spot">
          <aside className="user_profile_sidebar">
            <div className="bio_panel">
              <h3 id="bio_panel_name">{props.currentUser.first_name}!</h3>
              <br />
              ...
              <div className="bio_sections">
                ...
                <nobr>
                  From {city ? city + ", " + state : "Invalid Zip Code"}
                </nobr>
              </div>
            </div>
