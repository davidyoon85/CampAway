...
function Spot({ fetchSpot, match, history, currentUserId, openModal, spot, numReviews }) {

  useEffect(() => {
    window.scrollTo(0, 0);
    fetchSpot(match.params.spotId);
  }, [])

  const handleDelete = (e) => {
    e.preventDefault();

    deleteSpot(match.params.spotId).then(response => {
      history.push("/spots");
    });
  }
  ...
  if (Object.values(spot).length === 0) {
    ...
    const checkIn = from24to12(check_in);
    const checkOut = from24to12(check_out);
      ...
      <>
        <main className="spot_main_section">
          ...
          <div className="spot_page">
            ...
            <BookingContainer spotId={match.params.spotId} />
          </div>
        </main>
      </>
    );
